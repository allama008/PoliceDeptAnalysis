...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      actionButton(inputId = "input_action", label = "Show Interaction"),
      # Horizontal line ----
      tags$hr()
    ),
    ...
    mainPanel(
      tabsetPanel(
        tabPanel("Likes Interation", h4("Interaction w.r.t Likes"), plotOutput("likes")),
        tabPanel("Comments Interation", h4("Interaction w.r.t Comments"), plotOutput("comments")),
        tabPanel("Shares Interation", h4("Interaction w.r.t Shares"), plotOutput("shares"))
      )
    )
  )
)
...
getEngagementMonthly<-function(startdate,enddate)
{
  ...
  else
  {
    ...
    df$intlikes = as.double(as.numeric(df$likes)/as.numeric(no))
    df$intlikes =as.double(format(round(df$intlikes,2),nsmall=2))
    df$intcomments = as.double(as.numeric(df$comments)/as.numeric(no))
    df$intcomments =as.double(format(round(df$intcomments,2),nsmall=2))
    df$intshares = as.double(as.numeric(df$shares)/as.numeric(no))
    df$intshares =as.double(format(round(df$intshares,2),nsmall=2))
...
server <- function(input, output) {
  ...
  output$likes <- renderPlot({
    ggplot(engagement_df(), aes(x = dates, y = as.double(intlikes))) +
      geom_bar(stat = "identity") +
      labs(x = "Time Interval", y = "Interaction") +
      ggtitle("Interaction w.r.t ") +
      theme(axis.title.x = element_text(), axis.title.y = element_text(), 
            plot.title = element_text(size = 10, face = "bold")) +
      geom_text(aes(label=as.double(intlikes)), vjust=-0.3, color="black", size=3)
  })
  output$comments <- renderPlot({
    ggplot(engagement_df(), aes(x = dates, y = as.double(intcomments))) +
      geom_bar(stat = "identity") +
      labs(x = "Time Interval", y = "Interaction") +
      ggtitle("Interaction w.r.t Comments") +
      theme(axis.title.x = element_text(), axis.title.y = element_text(), 
            plot.title = element_text(size = 10, face = "bold")) +
      geom_text(aes(label=as.double(intcomments)), vjust=-0.3, color="black", size=3)
  })
  output$shares <- renderPlot({
    ggplot(engagement_df(), aes(x = dates, y = as.double(intshares))) +
      geom_bar(stat = "identity") +
      labs(x = "Time Interval", y = "Interaction") +
      ggtitle("Interaction w.r.t Shares") +
      theme(axis.title.x = element_text(), axis.title.y = element_text(), 
            plot.title = element_text(size = 10, face = "bold")) +
      geom_text(aes(label=as.double(intshares)), vjust=-0.3, color="black", size=3)
  })
}
