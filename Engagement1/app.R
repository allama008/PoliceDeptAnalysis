# ENGAGEMENT STATISTICS (Total Posts/Avg Posts/Weekend-Weekday Post/Post Type Division/
# Weekend-Weekday Likes)
  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("Weekend-Weekday Posts", h4("Plot"), plotOutput("week_post")),
        tabPanel("Post Type Division", h4("Plot"), plotOutput("post_type")),
        tabPanel("Weekend-Weekday Likes", h4("Plot"), plotOutput("week_like"))
      )
    )
  )
)
...
week_post_util <- function(engage_df)
{
  df <- data.frame(matrix(ncol = 4, nrow = 0))
  colnames(df) <- c("date", "post", "like","type")
  k <- 1
  type=c("Weekend","Weekday")
  for(i in 1:nrow(engage_df))
  {
    for(j in 1:2)
    {
      date_range <- paste(engage_df[i, 1],engage_df[i, 2],sep="\n")
      df[k, 1] <- date_range
      df[k, 2] <- engage_df[i, 21+j]
      df[k, 3] <- engage_df[i, 23+j]
      df[k, 4] <- type[j]
      k <- k + 1
    }
  }
...
server <- function(input, output) {
  ...
  graphical_df <- eventReactive(input$input_action, {week_post_util(engagement_df())})
  ...
  output$week_post <- renderPlot({
    ggplot(graphical_df(), aes(x=date, y = post, fill = type)) +
      geom_bar(stat="identity")+
      labs(x = "Time Interval", y = "No. of post") +
      ggtitle("No of Posts on Weekend and Weekday") +
      theme(axis.title.x = element_text(), axis.title.y = element_text(), 
            plot.title = element_text(size = 10, face = "bold")) +
      guides(color=guide_legend("Type:"))
  })
  
  output$week_like <- renderPlot({
    ggplot(graphical_df(), aes(x=date, y = like, fill = type)) +
      geom_bar(stat="identity")+
      labs(x = "Time Interval", y = "No. of Likes") +
      ggtitle("No of Likes on Weekend and Weekday") +
      theme(axis.title.x = element_text(), axis.title.y = element_text(), 
            plot.title = element_text(size = 10, face = "bold")) +
      guides(color=guide_legend("Type:"))
  })
